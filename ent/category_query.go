...
type CategoryQuery struct {
	...
	withNamedArticle map[string]*ArticleQuery
...
func (cq *CategoryQuery) QueryArticle() *ArticleQuery {
...
func (cq *CategoryQuery) WithArticle(opts ...func(*ArticleQuery)) *CategoryQuery {
	query := (&ArticleClient{config: cq.config}).Query()
...
func (cq *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := cq.withArticle; query != nil {
		if err := cq.loadArticle(ctx, query, nodes,
			func(n *Category) { n.Edges.Article = []*Article{} },
			func(n *Category, e *Article) { n.Edges.Article = append(n.Edges.Article, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range cq.withNamedArticle {
		if err := cq.loadArticle(ctx, query, nodes,
			func(n *Category) { n.appendNamedArticle(name) },
			func(n *Category, e *Article) { n.appendNamedArticle(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CategoryQuery) loadArticle(ctx context.Context, query *ArticleQuery, nodes []*Category, init func(*Category), assign func(*Category, *Article)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(category.ArticleTable)
		s.Join(joinT).On(s.C(article.FieldID), joinT.C(category.ArticlePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(category.ArticlePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.ArticlePrimaryKey[1]))
...
// WithNamedArticle tells the query-builder to eager-load the nodes that are connected to the "article"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (cq *CategoryQuery) WithNamedArticle(name string, opts ...func(*ArticleQuery)) *CategoryQuery {
	query := (&ArticleClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if cq.withNamedArticle == nil {
		cq.withNamedArticle = make(map[string]*ArticleQuery)
	}
	cq.withNamedArticle[name] = query
	return cq
}
